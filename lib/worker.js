// Plugin sandbox worker thread. Each plugin runs in its own worker + vm context
// with ONLY the Skystream host globals (no process, no require, no fs).
// The main thread kills this worker on call timeout (infinite loops) and respawns it.
...
const { code, manifest, defaults } = workerData; // defaults: { verifyUA }

const DEFAULT_UA =
  "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/119.0.0.0 Safari/537.36";

// ---------- network (fetch-based, exact Skystream shapes) ----------
...
// ponytail: mock, matching the official host's behavior
function solveCaptcha(siteKey, url) {
  return Promise.resolve("mock_captcha_token");
}

// ---------- entity classes (verbatim from the Skystream host) ----------
...
parentPort.on("message", async (msg) => {
  ...
  parentPort.postMessage({
    ...
    id: msg.id,
    ok: r.ok,
    value: r.ok ? safeClone(r.value) : undefined,
    error: r.error,
  });
});
