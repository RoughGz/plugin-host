// mini-dom: dependency-free HTML parser + selector engine for the plugin sandbox.
// Covers the exact API surface Skystream plugins use (see research):
//   querySelector/querySelectorAll (element, .class, #id, tag.class, descendant,
//   ">" child, ":contains(text)"), getAttribute, textContent, innerHTML, outerHTML,
//   tagName, className, parentElement, nextElementSibling, firstElementChild,
//   Array.from() over node lists, document.body.
// ponytail: hand-rolled on purpose — jsdom/cheerio don't support ":contains" and
// the needed surface is small; upgrade to jsdom if a plugin needs more CSS.
...
// Parse one compound selector into {tag, id, classes[], contains} (contains: string|null)
...
// Parse a full selector into alternatives of [[combinator, simple], ...]
// combinator: '>' (child) or ' ' (descendant)
...
// Reverse matching: the candidate element must match the LAST part; ancestors
// satisfy the earlier parts (the matched node is the end of the chain, not an
...
// parse_html(html, selector, attr) → [{text, html, attr, src, href}] (Skystream bridge)
...
// ---------- Dean Edwards packer unpacker (getAndUnpack) ----------
...
function unpackJs(js) {
  ...
  // p is a quoted JS string literal; strip the quotes
  const pRaw = args[0].trim();
  const pm = /^(['"])([\s\S]*)\1$/.exec(pRaw);
  const p = pm ? pm[2] : pRaw;
  const c = parseInt(args[2], 10) || 0;
  // k is either a pipe-joined string or `'a|b'.split('|')` (real packer output)
  ...
  const unescapeStr = hasD
    ...
    : (s) => s;
  let s = unescapeStr(p);
  // classic packer: p holds base-36 tokens; replace token -> k[token] (descending
  // order so multi-char tokens like 'a' don't clobber single-char ones)
  for (let i = c - 1; i >= 0; i--) {
...
module.exports = { parseHtml, parse_html, unpackJs, decodeEntities };
