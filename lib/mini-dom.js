...
// (element, .class, #id, tag.class, descendant, ">" child, ":contains(text)";
// getAttribute, textContent, innerHTML, outerHTML, tagName, className, parents,
// siblings, children, Array.from() over node lists, document.body).
// ponytail: hand-rolled on purpose — jsdom/cheerio lack ":contains" and the
// needed surface is small; upgrade to jsdom if a plugin needs more CSS.
...
// Parse one compound selector into {tag, id, classes[], contains}
...
function unpackJs(js) {
  ...
  const unescapeStr = hasD
    ...
    : (s) => s;
  let s = unescapeStr(p);
  // replace token -> k[token] descending so multi-char tokens don't clobber single-char ones
  for (let i = c - 1; i >= 0; i--) {
