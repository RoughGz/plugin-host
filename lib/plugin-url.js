...
// keep plugins.txt in sync with web/CLI adds so they survive a redeploy
function appendToPluginsTxt(file, url) {
  try {
    const existing = fs.existsSync(file)
      ? fs.readFileSync(file, "utf8").split("\n")
      : [];
    const trimmed = url.trim();
    if (existing.some((l) => l.trim() === trimmed)) return;
    const last = existing[existing.length - 1];
    fs.appendFileSync(file, (last && last.trim() ? "\n" : "") + trimmed + "\n");
  } catch (e) {
    console.warn("plugins.txt update failed:", e.message);
  }
}

function dropFromPluginsTxt(file, name) {
  try {
    ...
    const kept = fs
      ...
      .split("\n")
      .filter((l) => {
        const url = l.trim().replace(/#.*$/, "").trim();
        return !url || pluginNameFromUrl(url) !== name;
      });
