...
// v8.setFlagsFromString is a no-op for this flag at runtime — it must be set
// at process start (npm start passes it). Fail fast if it isn't active.
if (!process.execArgv.includes("--disallow-code-generation-from-strings")) {
  try {
    Function(""); // eslint-disable-line no-new-func
    console.error(
      "FATAL: --disallow-code-generation-from-strings not active — " +
        "plugin sandbox is escapable. Start with: node --disallow-code-generation-from-strings server.js",
    );
    process.exit(1);
  } catch (e) {
    // Function("") throws → the flag is active (set via NODE_OPTIONS or execArgv)
  }
}

const CALL_TIMEOUT_MS = (() => {
  const n = parseInt(process.env.CALL_TIMEOUT_MS || "120000", 10);
...
class PluginRuntime {
  ...
  spawn() {
    ...
    const w = new Worker(path.join(__dirname, "worker.js"), {
      ...
      env: {}, // plugins never need host env (GITHUB_TOKEN etc.)
      resourceLimits: {
